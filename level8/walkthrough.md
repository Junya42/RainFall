...
0x08048564  main # <----------- Only one function in our program
...
Dump of assembler code for function main:
   ...
   0x08048574 <+16>:    nop
   0x08048575 <+17>:    mov    0x8049ab0,%ecx              # service
   0x0804857b <+23>:    mov    0x8049aac,%edx              # auth
   ...
   0x080485a7 <+67>:    lea    0x20(%esp),%eax             # line[128]
   0x080485ab <+71>:    mov    %eax,(%esp)
   0x080485ae <+74>:    call   0x8048440 <fgets@plt>       # fgets(line, 128, stdin)
   0x080485b3 <+79>:    test   %eax,%eax
   0x080485b5 <+81>:    je     0x804872c <main+456>        # EOF → return
   ...
   0x080485cf <+107>:   repz cmpsb %es:(%edi),%ds:(%esi)   # strncmp(line, "auth ", 5)
   ...
   0x080485e2 <+126>:   jne    0x8048642 <main+222>        # not "auth " → skip
   ...
   0x080485fa <+150>:   movl   $0x0,(%eax)                 # *auth = 0
   0x08048600 <+156>:   lea    0x20(%esp),%eax
   0x08048604 <+160>:   add    $0x5,%eax                   # line + 5
   ...
   0x08048656 <+242>:   repz cmpsb %es:(%edi),%ds:(%esi)   # strncmp(line, "reset", 5)
   ...
   0x08048669 <+261>:   jne    0x8048678 <main+276>        # not "reset" → skip
   0x0804866b <+263>:   mov    0x8049aac,%eax
   0x08048670 <+268>:   mov    %eax,(%esp)
   0x08048673 <+271>:   call   0x8048420 <free@plt>        # free(auth)  (auth not nulled)
   0x08048678 <+276>:   lea    0x20(%esp),%eax
   0x0804867c <+280>:   mov    %eax,%edx
   0x0804867e <+282>:   mov    $0x8048825,%eax             # "servic"
   0x08048683 <+287>:   mov    $0x6,%ecx
   0x08048688 <+292>:   mov    %edx,%esi
   0x0804868a <+294>:   mov    %eax,%edi
   0x0804868c <+296>:   repz cmpsb %es:(%edi),%ds:(%esi)   # strncmp(line, "service", 6)
   ...
   0x0804869f <+315>:   jne    0x80486b5 <main+337>        # not "service" → skip
   0x080486a1 <+317>:   lea    0x20(%esp),%eax
   0x080486a5 <+321>:   add    $0x7,%eax                   # line + 7
   0x080486a8 <+324>:   mov    %eax,(%esp)
   0x080486ab <+327>:   call   0x8048430 <strdup@plt>      # strdup(line + 7)
   ...
   0x080486c9 <+357>:   repz cmpsb %es:(%edi),%ds:(%esi)   # strncmp(line, "login", 5)
   ...
   0x080486dc <+376>:   jne    0x8048574 <main+16>         # not "login" → loop
   0x080486e2 <+382>:   mov    0x8049aac,%eax              # auth
   0x080486e7 <+387>:   mov    0x20(%eax),%eax             # *(auth + 32)
   0x080486ea <+390>:   test   %eax,%eax
   0x080486ec <+392>:   je     0x80486ff <main+411>        # 0 → Password
...
The program loops, reads a line, and looks at the start of that line.
### auth <name>

- `malloc(4)` → the global `auth` points at this tiny chunk
- copies the name into it (max 30 characters)
- `*auth` is first set to 0, this creates the account

### reset

- `free(auth)`
- the global `auth` is not set back to `NULL`
- the chunk is freed, the pointer remains

### service <text>

- `strdup(text)` → copies the text into a new heap chunk, the global service points at it
- Each call = one more chunk, usually packed right after the previous one

### login

- reads 4 bytes at auth + 32
- ≠ 0 → system("/bin/sh")
- = 0 → prints Password: it does not check a real password, it only checks whether that memory slot is empty

The (nil), (nil) line

At the start of each loop: printf("%p, %p", auth, service).

You see where the two chunks are. When service == auth + 0x20, you are sitting on the slot login reads.

## Solution
auth lives in a 16-byte chunk because in 32bits libc
Each allocation is stored like this [prev_size 4][size 4][data]
------------------------------ header = 8 bytes
Then everything is align to 8 bytes so a chunk = 8 header bytes + data = 16 bytes minimum

Since the `service` cmd doesn't rewrite its own memory space and adds a new one, typing the `service` cmd twice will add 2 chunks = 32 bytes, enough to make `auth[32]` non null
We can also just type `service` followed by min 16 characters to force to force a chunk of 32 bytes that will cover up for `auth[32]`
